...
import { FEED_FETCH_TIMEOUT_MS, FEED_CONCURRENCY } from './types';
...
// ============================================================================
// RSS/Atom Parsing Helper Functions
// ============================================================================
...
async function fetchFeed(feed: { name: string; xmlUrl: string; htmlUrl: string }): Promise<Article[]> {
  try {
    ...
    const response = await fetch(feed.xmlUrl, {
      ...
      headers: {
        'User-Agent': 'DeepDigest/2.0 (RSS Reader)',
...
export async function fetchAllFeeds(feeds: typeof RSS_FEEDS, sendEvent?: SendEvent): Promise<Article[]> {
  ...
  for (let i = 0; i < feeds.length; i += FEED_CONCURRENCY) {
    ...
    sendEvent?.('progress', { step: 1, current: progress, total: feeds.length });
  }
  sendEvent?.('log', { message: `Fetched ${allArticles.length} articles from ${successCount} feeds (${failCount} failed)` });
  return allArticles;
}
